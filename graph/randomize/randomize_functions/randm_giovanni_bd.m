...
for i = 1 : length(ind_rm)
    [~, rm_col] = ind2sub(size(A),ind_rm(i));  % find old row and col
...
% Below lines of coded are no longer needed since miswired edges are taken
% care of in the above code snippet.
% e(ind_mw) = 0;
% c(ind_mw) = 0;
% r(ind_mw) = 0;
...
% for i = 1 : length(ind_rm)
%     [~, rm_j] = ind2sub(size(A),ind_rm(i));
%     
%     possible = find(B(:, rm_j) == 0);  % Find possible nodes to rewire to
%     idx = randperm(length(possible), 1);  % Pick one index
%     rm_i_new = possible(idx);
%     B(rm_i_new, rm_j) = 1;
% end
