...
xticks = linspace(x_start_value, 0.14, 5);
yticks = linspace(y_min_value, 0.4, 4);
...
h_max = maxis2d([x_start_value*0.95 x_end_value*1.02], [-0.02 y_max_value*1.5],...
    'X0', x_start_value,...
    'XTicks',x_values,...
    'XTickLabels',{sprintf('$%.3f$', x_values(1)),'',...
    sprintf('$%.3f$', x_values(3)),'',...
    sprintf('$%.3f$', x_values(5)),'',sprintf('$%.3f$', x_values(7))},...
    'YTicks',yticks,...
...
for h3_loop = 1:1:length(h3)
    a = h3(h3_loop);
    set(a,'YData',a.YData - 0.015)
end
for h4 = h_max.xticklabels
    set(h4,'Position',get(h4,'Position')-[0 .02 0]);
end
end
