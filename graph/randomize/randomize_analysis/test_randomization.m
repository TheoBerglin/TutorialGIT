...
for runi = 1: length(test_func1_array)
    ...
    for i = 1:length(densities)
        ...
        if exist(save_file_1, 'file') % Ground truth available
            ...
            disp('GT not loaded')
            gm_list_new = run_randomization(A, type, test_func_1, n_randomizations);
            gm_list_1 = [gm_list_1, gm_list_new];
            save(save_file_1, 'gm_list_1'); % Save ground truth
        end
        
        %% Run new randomization function
        if exist(save_file_2, 'file') % Ground truth available
            d = load(save_file_2);
            gm_list_2 = d.gm_list_2;
            disp('GT not loaded')
            gm_list_2_new = run_randomization(A, type, test_func_2, n_randomizations);
            gm_list_2 = [gm_list_2, gm_list_2_new];
            save(save_file_2, 'gm_list_1'); % Save ground truth
        end
